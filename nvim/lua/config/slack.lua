...
local function ensure_layout()
  ...
  -- Reply buffer (left, where user types)
  if not state.reply_buf or not vim.api.nvim_buf_is_valid(state.reply_buf) then
    ...
    -- :w → send (keeps window open), :wq → send then close
  ...
  vim.api.nvim_win_set_buf(0, state.reply_buf)
  state.reply_win = vim.api.nvim_get_current_win()
  vim.api.nvim_set_option_value("wrap", true, { win = state.reply_win })
  vim.api.nvim_set_option_value("linebreak", true, { win = state.reply_win })
  vim.api.nvim_win_set_width(state.reply_win, math.floor(vim.o.columns * 0.35))

  -- Thread buffer (right, readonly reference)
  vim.cmd("vsplit")
  if not state.thread_buf or not vim.api.nvim_buf_is_valid(state.thread_buf) then
  ...
  vim.api.nvim_set_option_value("wrap", true, { win = state.thread_win })
  vim.api.nvim_set_option_value("linebreak", true, { win = state.thread_win })
end
